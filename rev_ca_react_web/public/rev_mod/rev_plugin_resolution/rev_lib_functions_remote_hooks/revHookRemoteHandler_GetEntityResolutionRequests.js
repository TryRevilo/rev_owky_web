var revHookRemoteHandlerCallback = async (revVarArgs) => {
    if (revVarArgs && revVarArgs.revRemoteHookMethods) {
        ...
        if (revLoggedInEntityGUID >= 1) {
            let revEntitySubTypesArr = await revRemoteHookMethods.revPersReadRevEntities_By_Subtype("rev_store_sale_resolution");
            ...
            for (let i = 0; i < revEntitySubTypesArr.length; i++) {
                ...
                revQuestionEntityStatsPassVarArgs["revResolution"] = revCurrEntitySubtype;
                revQuestionEntityStatsPassVarArgs["revRemoteHookMethods"] = revRemoteHookMethods;

                let revEntityQuestionStatData = await revRemoteHookMethods.revPluginHookRemoteEnvironment_ResolutionStats(revQuestionEntityStatsPassVarArgs);
