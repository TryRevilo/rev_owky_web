var revFormViewWidget = async (revVarArgs) => {
    if (!revVarArgs) {
        console.log("ERR -> revAskQuestionForm_Widget -> !revVarArgs");
        return;
    }
    ...
    if (revVarArgs && revVarArgs._revEntitySubType && revVarArgs._revEntitySubType.localeCompare("rev_question") == 0) {
    ...
    let revPageHeaderTxt = "Ask A QuEsTioN" + revServiceDescription;

    if (revRemoteEntityGUID > 0) {
        revPageHeaderTxt = "updAte QuEsTioN" + revServiceDescription;
    }

    let revPageHeader = window.revPageHeader(revPageHeaderTxt);
    /** REV END HEADER AREA */

    let revQuestionTitleInput_Id = "revQuestionTitleInput_Id_" + window.revGenUniqueId();

    let revQuestionTitleInput = window.revInputText({
        "revId": revQuestionTitleInput_Id,
        "revIcon": '<div class="revQuestionFormTitleIcon"><i class="fas fa-exclamation revFontSizeNormal"></i></div>',
        "revTitle": "",
        "revPlaceholderText": "QuesTioN TiTLE . . .",
    });

    window.revSetInterval(revQuestionTitleInput_Id, () => {
        document.getElementById(revQuestionTitleInput_Id).value = revEntityName;
    });

    let revSubmitQuestion_Id = "revSubmitQuestion_Id_" + window.revGenUniqueId();

    let revFormSubmitTab = window.revFormSubmitTab({
        "revId": revSubmitQuestion_Id,
    ...
    let revQuestionDetailsArea_Id = "revQuestionDetailsArea_Id";

    let revEntityDescQuill;

    window.revSetInterval(revQuestionDetailsArea_Id, () => {
        revEntityDescQuill = window.revNewQuill(revQuestionDetailsArea_Id, "QuEsTioN DeTAiLs");
    ...
    revEntityIconSelectionAreaVarArgs["revSelectFileText"] = "picTuREs";
    ...
    let revSavePersEntity = async () => {
        ...
        revPersEntity._revEntitySubType = "rev_question";
        ...
        let revPersEntityName = window.revGetTextInputVal(revQuestionTitleInput_Id);
    ...
    <div class="revFlexContainer">
        ...
        <div class="revFlexContainer revNewQuestionWidgetContainer">
            ${revQuestionTitleInput}
            <div class="revQuestionDetailsArea">
                <div id="${revQuestionDetailsArea_Id}" class="revQuillPubArea"></div>
                <div class="revFlexWrapper revQuestionTagsFormWrapper">
                    <div class="revSmall-H-Line"></div>
                    <div class="revFlexContainer revQuestionTagsFormContainer">${revTagsInputForm}</div>
                </div>
            </div>
            ${await revGetEditablePicsAlbum(revInfoEntity)}
            <div class="revFlexWrapper revSelectQuestionPicsWrapper">${revEntityIconSelectionArea}</div>
            <div class="revFlexWrapper revQuestionFormFooterWrapper">
                ${revFormSubmitTab}
            </div>
        </div>
    </div>
