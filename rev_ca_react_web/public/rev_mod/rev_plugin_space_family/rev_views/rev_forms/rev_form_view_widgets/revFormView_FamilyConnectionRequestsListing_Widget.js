var revFormViewWidget = async (revVarArgs) => {
    ...
    let revNoticiaItemListingSummuryWrapper = (revConnEntity) => {
        ...
        let revFamilyReqConnListing_Id = revNoticiaEntityGUID + "_revFamilyReqConnListing_Id_" + window.revGenUniqueId();
        ...
        let revCheckBoxCallback = (revCBVarArgs) => {
            if (revCBVarArgs.revCheckBoxChecked) {
                revSelectedGUIDsArr.push(revCBVarArgs.revCheckBoxVal);
            } else {
                window.revRemoveArrElement(revSelectedGUIDsArr, revCBVarArgs.revCheckBoxVal);
            }

            console.log("revSelectedGUIDsArr : " + JSON.stringify(revSelectedGUIDsArr));
        };
        ...
        window.revSetInterval(revAcceptTab_Id, () => {
            document.getElementById(revAcceptTab_Id).addEventListener("click", (event) => {
                revActionCallbackAction({
                    "revNoticiaEntityGUID": revNoticiaEntityGUID,
                    "revFamilyReqConnListing_Id": revFamilyReqConnListing_Id,
                });
            });
        });

        return `
            <div id="${revFamilyReqConnListing_Id}" class="revFlexWrapper revNoticiaItemListingContentWrapper">
    ...
    try {
        let revPath = `${window.REV_SITE_BASE_PATH}/rev_api?rev_logged_in_entity_guid=${window.REV_LOGGED_IN_ENTITY_GUID}&revPluginHookContextsRemoteArr=revHookRemoteHandler_GetFamilyConnRequestEntities`;
