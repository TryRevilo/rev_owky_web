var revHookRemoteHandlerCallback = async (revVarArgs) => {
    if (revVarArgs && revVarArgs.revRemoteHookMethods) {
        ...
        if (revRemoteHookMethods.revJSONArrContains_NameId(revVarArgs.filter, "revContextView_ConnectionRequest")) {
            return revVarArgs;
        }
        ...
        if (revNoticiasTotCount > 0 == true) {
            revVarArgs.filter.push({ "revNameId": "revContextView_ConnectionRequest", "revNoticiaCount": revNoticiasTotCount });
        }
    }
